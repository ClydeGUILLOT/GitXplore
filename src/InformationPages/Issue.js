...
export default class Issue extends Component {
    ...
    async getGithubInfo() {
        ...
        console.log(`https://api.github.com/repos/${this.username}/${this.repoName}/issues/${this.issueNumber}`);
        this.setState({
                ...
                {isLoading ? <ActivityIndicator/> : (
                    <View>
                        <View style={{alignItems: "center"}}>
                            <Text style={{fontSize: 23, fontWeight: "bold", padding: 15}}>{issueInfo.title}</Text>
                            <Text style={{fontSize: 18, fontStyle: "italic", padding: 15}}>{issueInfo.state}</Text>
                            ...
                            <TouchableOpacity onPress={() => this.props.navigation.push('User', {
                                ...
                                <Text style={{fontSize: 17, padding: 15}}>Submitted by: {issueInfo.user.login}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
